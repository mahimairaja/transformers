...
class RepetitionPenaltyLogitsProcessor(LogitsProcessor):
    r"""
    [`LogitsProcessor`] prevents repetition of previous tokens through an exponential penalty. This technique invloves adjusting the scores assigned to previously generated tokens, discouraging repetition. This technique shares some similarities with with coverage mechanisms and other aimed at reducing repetition. 
    During the text generation process, the probability distribution for the next token is determined using a formula that incorporates token scores based on their occurrence in the generated sequence. Tokens with higher scores are less likely to be selected. The formula takes into account a fair bunch of variables which can be further seen in the [paper](https://arxiv.org/pdf/1909.05858.pdf).

    <Tip>

    It is worth noting that the effectiveness of penalized sampling relies on the model's ability to learn a reliable distribution of tokens during its training.

    </Tip>

    Args:
        repetition_penalty (`float`):
            The parameter for repetition penalty. 1.0 means no penalty. According to the paper a penalty of around 1.2 yields a good balance between truthful generation and lack of repetition. 
